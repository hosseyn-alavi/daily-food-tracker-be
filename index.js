...
app.post("/api/login", function (req, res) {
    ...
    fs_1.default.readFile(filePath, "utf8", function (err, data) {
        ...
        if (user) {
            // Generate JWT token
            var token = jsonwebtoken_1.default.sign({ username: req.body.username }, secretKey, {
                expiresIn: "1h",
            });
...
app.get("/api/records/:date", authenticateToken, function (req, res) {
    ...
    var filePath = path_1.default.join(__dirname, "data", "records", "".concat(date, ".json"));
    fs_1.default.readFile(filePath, "utf8", function (err, data) {
        if (err) {
...
app.post("/api/records/:date", authenticateToken, function (req, res) {
    ...
    fs_1.default.readFile(filePath, "utf8", function (err, data) {
        var records = [];
        if (data) {
            console.log.apply(console, __spreadArray(["file: index.ts:170 ~ data:"], JSON.parse(data), false));
